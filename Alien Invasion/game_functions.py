...
def create_fleet(ai_settings, stats, screen, ship, aliens):
    ...
    if stats.level % 2 == 0:
        ai_settings.set_boss_image_size()
        create_alien(ai_settings, screen, aliens, 1, 0)
    else:
        ai_settings.set_default_image_size()
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    if stats.level % 2 == 0 and ai_settings.boss_health > 0:
        collisions = pygame.sprite.groupcollide(bullets, aliens, True, False)
        if collisions:
            ai_settings.boss_damage()
            print(ai_settings.boss_health)
    else:
        collisions = pygame.sprite.groupcollide(bullets, aliens, True, True)
        if collisions:
            for aliens in collisions.values():
                if stats.level % 2 == 0:
                    stats.score += ai_settings.alien_points * 100 * len(aliens)
                    sb.prep_score()
                else:
